...
\textit{ChurnGuard AI}, a system that combines a classical
supervised machine learning pipeline with a modern agentic AI layer.
The predictive layer trains and compares three classifiers — Logistic Regression,
...
An agentic layer extends the system with a LangGraph-based agentic workflow
...
\begin{enumerate}
  \item \textbf{Predictive ML Pipeline:} Design, train, and
  ...
  \item \textbf{Agentic AI Advisor:} Wrap the ML output in
...
the agentic architecture; Section VI concludes with
...
The agentic layer wraps the ML pipeline with a multi-node LangGraph
...
Figure~\ref{fig:agent_arch} illustrates the full agentic pipeline.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.48\textwidth]{figures/agent_pipeline.png}
  \caption{Agentic pipeline. SHAP attributions and RAG
